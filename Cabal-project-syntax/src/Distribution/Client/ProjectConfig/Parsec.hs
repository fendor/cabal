...
type ProjectConfigSkeleton = CondTree ConfVar [ProjectConfigPath] [ProjectConfigParseResult]

singletonProjectConfigSkeleton :: ProjectConfig -> ProjectConfigSkeleton
singletonProjectConfigSkeleton x = CondNode [ParsedProjectConfig x] mempty mempty

singletonProjectConfigPath :: ProjectConfigPath -> ProjectConfigSkeleton
singletonProjectConfigPath x = CondNode [ImportUri x] mempty mempty
...
parseProject rootPath cacheDir verbosity configToParse = do
  ...
  parseProjectSkeleton cacheDir verbosity projectDir projectPath configToParse
...
parseProjectSkeleton
  ...
  -- ^ The contents of the file to parse
  -> IO (ParseResult ProjectFileSource ProjectConfigSkeleton)
parseProjectSkeleton cacheDir verbosity projectDir source (ProjectConfigToParse bs) = do
  ...
  where
          ...
          ( \importLoc -> do
              ...
              if isCyclicConfigPath normLocPath
                then pure $ parseFatalFailure pos (render $ cyclicalImportMsg normLocPath)
                  ...
                  let
                    fs :: ParseResult ProjectFileSource ProjectConfigSkeleton
                    fs =
                          (\z -> CondNode [ParsedProjectConfig z] [normLocPath] mempty)
                      <$> fieldsToConfig normSource (reverse acc)

                  rest <- go [] xs
                  pure . fmap mconcat . sequence $ [fs, pure (singletonProjectConfigPath normLocPath) ,rest]
          )
    ...
    --     -- TODO: We should replace this with hole that gets filled later during resolving
    ...
    modifiesCompiler :: [ProjectConfigParseResult] -> Bool
    modifiesCompiler pc = any mygo pc
      where
        mygo (ImportUri{}) = False
        mygo (ParsedProjectConfig p) = isSet projectConfigHcFlavor || isSet projectConfigHcPath || isSet projectConfigHcPkg
          where isSet f = f (projectConfigShared p) /= NoFlag
    ...
    sanityWalkBranch :: CondBranch ConfVar [ProjectConfigPath] [ProjectConfigParseResult] -> ParseResult ProjectFileSource ()
